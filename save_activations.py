...
def get_args_parser():
    ...
    ###############################################TODO
    parser.add_argument("--last_token", default=True, action="store_true")
    parser.add_argument("--take_n_parquets", default=0, type=int,
                        help="Only use first N parquet shards for imagenet parquet layout (0=all).")
    parser.add_argument("--max_samples", default=0, type=int,
                        help="Only use first M samples after loading (0=all).")
...
def save_activations(activations, count, split, save_count, args):
    ...
    ###############################################TODO
        ...
        activations_tensor = activations_tensor[:, -1, :]
    ###############################################TODO
    elif args.mean_pool:
    ...
    torch.save(activations_tensor.cpu(), save_path)
    ###############################################TODO
